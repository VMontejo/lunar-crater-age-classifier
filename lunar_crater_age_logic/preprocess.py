...
def augment_image_tf(image: tf.Tensor) -> tf.Tensor:
    ...
    if tf.random.uniform([]) > 0.5:
        ...
        new_size = tf.cast(tf.constant(IMAGE_SIZE, dtype=tf.float32) * zoom, tf.int32)
        new_size = tf.maximum(new_size, IMAGE_SIZE)
...
def create_tf_dataset(
    ...
    if balanced:
        ...
        file_paths, labels = create_weighted_sampler_tf(
            data_dir, subset, seed
        )
    else:
        ...
        for idx, cname in enumerate(CLASS_NAMES):
            files = list((base / cname).glob("*.jpg"))
    ...
    if shuffle and not weighted_sampling:
        ds = ds.shuffle(min(total, 2000), seed=seed)

    ds = ds.map(
        lambda fp, lbl: preprocess_single_image_tf(
            fp, lbl, normalization, model_type, augment
        ),
